...
static bool __avdVulkanCreateSurface(AVD_Vulkan* vulkan, GLFWwindow* window)
{
...
static bool __avdVulkanCreateDevice(AVD_Vulkan *vulkan)
{
    AVD_ASSERT(vulkan != NULL);

    int32_t graphicsQueueFamilyIndex = __avdVulkanFindQueueFamilyIndex(vulkan->physicalDevice, VK_QUEUE_GRAPHICS_BIT, vulkan->swapchain.surface, -1);
...
bool avdVulkanInit(AVD_Vulkan *vulkan, AVD_Window* window)
{
    ...
    AVD_CHECK(__avdVulkanCreateSurface(vulkan, window->window));
    AVD_CHECK(__avdVulkanPickPhysicalDevice(vulkan));
    AVD_CHECK(__avdVulkanCreateDevice(vulkan));
    ...
    AVD_CHECK(avdVulkanSwapchainCreate(vulkan, window));
    AVD_CHECK(avdVulkanRendererCreate(vulkan));
    AVD_CHECK(avdVulkanPresentationInit(vulkan));
    return true;
}
...
void avdVulkanShutdown(AVD_Vulkan *vulkan)
{
    ...
    avdVulkanPresentationDestroy(vulkan);
    avdVulkanRendererDestroy(vulkan);
    ...
    avdVulkanSwapchainDestroy(vulkan);
