    ...
    AVD_UInt32 subdivisions)
{
    ...
    // Create base octahedron vertices (already normalized)
    ...
    const uint32_t baseIndices[] = {
        0, 4, 3,  0, 3, 5,  0, 5, 2,  0, 2, 4, // Top hemisphere
        1, 3, 4,  1, 5, 3,  1, 2, 5,  1, 4, 2  // Bottom hemisphere
    };
    for(size_t i = 0; i < AVD_ARRAY_COUNT(baseIndices); ++i) {
        avdListPushBack(&localIndices, &baseIndices[i]);
    }

    // Subdivide faces iteratively
    for (uint32_t i = 0; i < subdivisions; ++i) {
        AVD_MeshGenMidpointCache cache;
        __avdMeshGenCacheCreate(&cache, localIndices.count); // A reasonable starting capacity
        ...
        for (size_t j = 0; j < localIndices.count; j += 3) {
            ...
            uint32_t newFaces[] = {i1, m12, m31, i2, m23, m12, i3, m31, m23, m12, m23, m31};
