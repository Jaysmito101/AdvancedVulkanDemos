...
typedef struct PS_ProloguePushConstants {
    float windowWidth;
    float windowHeight;
    float sphereRadius;
    float time;
    ...
    float rotation;
} PS_ProloguePushConstants;

bool psScenesPrologueInit(PS_GameState *gameState) {
    PS_ASSERT(gameState != NULL);
    PS_PrologueScene *scene = &gameState->scene.prologueScene;
    ...
    pushRange.offset = 0;
    pushRange.size = sizeof(PS_ProloguePushConstants);
    ...
    if (vkCreatePipelineLayout(gameState->vulkan.device, &layoutInfo, NULL, &scene->pipelineLayout) != VK_SUCCESS) {
        PS_LOG("Failed to create prologue pipeline layout\n");
        vkDestroyShaderModule(gameState->vulkan.device, vert, NULL);
    ...
    if (vkCreateGraphicsPipelines(gameState->vulkan.device, VK_NULL_HANDLE, 1, &pipelineInfo, NULL, &scene->pipeline) != VK_SUCCESS) {
        PS_LOG("Failed to create prologue graphics pipeline\n");
        return false;
    }
