...
def create_font_asset(file_path, output_dir, temp_dir, msdf_exe_path, git_root):
    ...
    source_source = [
        ...
        f"// Name: {font_name}",
        f"\n",
        generate_c_code_for_bytes(atlas_image_bytes, f"PRIV_avdAssetFontAtlas__{font_name}_{font_hash}"),
        f"\n",
        f"const uint8_t* avdAssetFontAtlas_{font_name}(size_t* size) {{",
        f"    if (size != NULL) {{",
        f"        *size = sizeof(PRIV_avdAssetFontAtlas__{font_name}_{font_hash});",
        f"    }}",
        f"   return &PRIV_avdAssetFontAtlas__{font_name}_{font_hash}[0];",
        f"}};",
        f"\n",
        generate_c_code_for_avd_font_atlas(font_metrics_data, f"PRIV_avdAssetFontMetrics__{font_name}_{font_hash}"),
        f"\n",
        f"const AVD_FontAtlas* avdAssetFontMetrics_{font_name}(void) {{",
        f"   return &PRIV_avdAssetFontMetrics__{font_name}_{font_hash};",
        f"}};",
        f"\n"
    ]
...
def create_image_asset(file_path, output_dir):
    ...
    source_source = [
        ...
        f"// Name: {image_name}",
        f"\n", 
        generate_c_code_for_bytes(image_bytes, f"PRIV_avdAssetImage__{image_name}_{image_hash}"),
        f"\n",
        f"const uint8_t* avdAssetImage_{image_name}(size_t* size) {{",
        f"    if (size != NULL) {{",
        f"        *size = sizeof(PRIV_avdAssetImage__{image_name}_{image_hash});",
        f"    }}",
        f"   return &PRIV_avdAssetImage__{image_name}_{image_hash}[0];",
        f"}};",
        f"\n"
    ]
...
def create_shader_asset(file_path, output_dir) -> bool:
    ...
    source_source = [
        ...
        f"// Language    : {extension}",
        f"\n", 
        generate_c_code_for_string(shader_text, f"PRIV_avdAssetShader__{shader_name}_{shader_hash}"),
        f"\n",
        f"const char* avdAssetShader_{shader_name}(void) {{",
        f"   return PRIV_avdAssetShader__{shader_name}_{shader_hash};",
        f"}};",
        f"\n",
        f"AVD_ShaderStage avdAssetShaderStage_{shader_name}(void) {{",
...
def create_text_asset(file_path, output_dir):
    ...
    src = [
        ...
        "// Auto-generated. Do not edit.",
        "",
        generate_c_code_for_string(text_data, f"PRIV_avdAssetText__{name}_{text_hash}"),
        "",
        f"const char* avdAssetText_{name}(void) {{",
        f"    return PRIV_avdAssetText__{name}_{text_hash};",
        f"}}",
        ""
    ]
